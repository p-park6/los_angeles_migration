...
la_stayed <- top_pooled[1,] %>% 
  mutate(travel_status = "Stayed") %>% 
  select(travel_status, n)

#subset for all other observations
la_movedin <- top_pooled[2:9,] %>%
  summarise(n = sum(n)) %>% 
  mutate(travel_status = "Moved In")


#combine two datasets into one to make visualization
compare_movein_pool <- full_join(la_stayed, la_movedin) %>% 
...
la_bar <- ggplot(data = compare_movein_pool, aes(x = "", y = n, fill = travel_status)) + #specify what datasets and aes I want to look at
  geom_bar(stat = "identity", width = 0.5, color="white") + #specify the separation between the different colors by differating them by white
  coord_polar("y", start=0) + #set this parameter to create a pie chart
  scale_fill_brewer(palette=4) +
  geom_label_repel(aes(label = paste0(pop_percent, "%")), position = position_stack(vjust=0.5)) + #label for those that stayed in LA
  #geom_text(aes(label = paste0(n, "%")), position = position_stack(vjust=0.5), vjust = 5) + #label for those that moved into LA
  theme_void() +
...
bot_cities_bar_controlled_y <- ggplot() +
  geom_col(data = migrate_pooled, aes(fill = city_state, y = n, x = combined), width = 0.5, position = position_stack(vjust = 1), fill = "black") + #create columns to make them stack on top of each other
